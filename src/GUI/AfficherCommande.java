 ...
 * @author Mega-PC
 */
public class AfficherCommande extends BaseForm{
         public AfficherCommande(Resources res) {
 ...
 //user User = Session.getCurrentSession();
        ...
        java.util.List<Commande> commande = ServiceCommande.getInstance().getMesCommandes(User);
        MultiButton[] cmps = new MultiButton[commande.size()];
        ...
        InfiniteScrollAdapter.createInfiniteScroll(this.getContentPane(), () -> {
            ...
            for (Commande comm : commande) {
                cmps[i] = new MultiButton(comm.getId_commande()+"   "+comm.getPrix_total()+"    "+comm.getDate());
                //cmps[i] = new MultiButton(String.valueOf(prod.getPrix_prod()));
                //cmps[i].setEmblem(FontImage.createMaterial(FontImage.MATERIAL_ADD_CIRCLE, s));
                cmps[i].addActionListener(cliqueEvent -> {
                    //idUser
                    //id
                  DetailsCommande.CommandeS=comm;
                  
                  Form bdf = new DetailsCommande();
                  bdf.show();
